...
while True:
    ...
    if cv2.waitKey(1) & 0xFF == ord('c'):
        if len(base_coor_queue) < 3 or len(target_coor_queue) < 3:
            print("Eliga 3 coordenadas en cada imagen")
        else: 
            pts1 = get_affine_array(base_coor_queue)
            pts2 = get_affine_array(target_coor_queue)
            transform = cv2.getAffineTransform(pts1,pts2)
            print(transform)
            result = cv2.warpAffine(image, transform,(cols, rows))
